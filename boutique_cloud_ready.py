...
def get_mongo_client():
    try:
        ...
        if not mongo_uri:
            st.error("⚠️ MONGO_URI not found! Please add it to Streamlit secrets or .env file")
            st.stop()
...
def get_next_id():
    """Generate auto-incrementing ID using a counter collection"""
    db = get_db()
...
def calculate_metrics():
    ...
    if df.empty:
        return {'total_sales': 0, 'total_profit': 0, 'total_pending': 0, 'delayed_payments': 0}
...
def get_all_customers():
    ...
    pipeline = [
        ...
        {"$group": {"_id": "$customer_name", "customer_phone": {"$first": "$customer_phone"}}},
        {"$sort": {"_id": 1}}
    ]
    results = list(col.aggregate(pipeline))
    return [(r["_id"], r.get("customer_phone", "")) for r in results]

def get_customer_details(customer_name):
    col = get_collection()
    doc = col.find_one({"customer_name": customer_name}, {"customer_name": 1, "customer_phone": 1})
    if doc:
        return (doc.get("customer_name"), doc.get("customer_phone", ""))
    return None
...
def add_sale():
    ...
    if customer_type == "👥 Existing Customer":
        ...
        if customers:
            ...
            selected_customer = st.selectbox("Select Existing Customer", customer_options)
...
def review_accounts():
    ...
    display_cols = ["id", "customer_name", "customer_phone", "sale_date", "product_category",
    ...
    display_df = display_df.rename(columns={
        "id": "ID", "customer_name": "Customer", "customer_phone": "Phone",
        "sale_date": "Date", "product_category": "Category",
        "buying_price": "Buy Price", "selling_price": "Sell Price",
        "amount_paid": "Paid", "pending_amount": "Pending",
        "payment_method": "Payment", "delay_status": "Delayed"
    })
    ...
    with col_csv:
        ...
        st.download_button("📥 Download as CSV", data=csv,
                           file_name=f"sales_report_{date.today()}.csv", mime="text/csv",
                           use_container_width=True)
    with col_excel:
        excel_file = export_to_excel(filtered_df)
        st.download_button("📊 Download as Excel", data=excel_file,
            ...
            with col_c:
                if st.button("✅ Received", key=f"pay_{row['id']}"):
                    get_collection().update_one(
                        {"id": row["id"]},
                        {"$set": {"payment_received": 1, "amount_paid": row["selling_price"], "pending_amount": 0}}
                    )
...
def update_transaction():
    ...
    if search_name or search_id > 0:
        mongo_col = get_collection()
        if search_name:
            docs = list(mongo_col.find({"customer_name": {"$regex": search_name, "$options": "i"}}, {"_id": 0}))
        else:
            docs = list(mongo_col.find({"id": int(search_id)}, {"_id": 0}))
            ...
            with st.form("update_form"):
                ...
                with col1:
                    ...
                    categories = ["Sarees", "Salwar Suits", "Lehengas", "Kurtis", "Western Wear",
                                  "Accessories", "Kids Wear", "Other"]
                    cat_index = categories.index(sale.get("product_category", "Other")) if sale.get("product_category") in categories else 0
                    new_category = st.selectbox("Category", categories, index=cat_index)

                with col2:
                    new_amount_paid = st.number_input("Update Amount Paid (₹)",
                                                       value=float(sale["amount_paid"]), min_value=0.0, step=100.0)
                    mark_delay = st.checkbox("Mark as Delayed Payment", value=bool(sale["delay_status"]))
                    payment_methods = ["Cash", "UPI", "Card", "Bank Transfer", "Part Payment"]
                    pm_index = payment_methods.index(sale.get("payment_method", "Cash")) if sale.get("payment_method") in payment_methods else 0
                    new_payment_method = st.selectbox("Payment Method", payment_methods, index=pm_index)
                ...
                if delete_button:
                    get_collection().delete_one({"id": selected_id})
...
def analytics_dashboard():
    ...
    with col2:
        st.metric("Total Revenue", f"₹ {df['selling_price'].sum():,.0f}")
    with col3:
        st.metric("Total Profit", f"₹ {df['profit'].sum():,.0f}")
    with col4:
        st.metric("Total Pending", f"₹ {df['pending_amount'].sum():,.0f}")
    with col5:
        st.metric("Avg Profit %", f"{df['profit_margin'].mean():.1f}%")
    ...
    with tab1:
        ...
        with col1:
            ...
            monthly_data = df.groupby("month").agg({"selling_price": "sum", "profit": "sum"}).reset_index()
            fig1 = go.Figure()
            fig1.add_trace(go.Bar(x=monthly_data["month"], y=monthly_data["selling_price"], name="Revenue", marker_color='#667eea'))
            fig1.add_trace(go.Bar(x=monthly_data["month"], y=monthly_data["profit"], name="Profit", marker_color='#764ba2'))
            fig1.update_layout(title="Monthly Revenue vs Profit", barmode='group', height=400)
            st.plotly_chart(fig1, use_container_width=True)
        with col2:
            daily_sales = df.groupby("sale_date").size().reset_index(name="count")
            fig2 = px.line(daily_sales, x="sale_date", y="count", title="Sales Trend Over Time", markers=True)
    ...
    with tab2:
        ...
        with col1:
            ...
            fig3 = px.bar(top_customers, x="selling_price", y="customer_name", orientation='h',
                          title="Top 10 Customers by Revenue", color="selling_price", color_continuous_scale="Purples")
        ...
        with col2:
            ...
            if len(customer_pending) > 0:
                st.dataframe(customer_pending.reset_index().rename(
    ...
    with tab3:
        ...
        with col1:
            ...
            fig4 = px.pie(category_sales, values="count", names="product_category",
                          title="Sales Distribution by Category", color_discrete_sequence=px.colors.sequential.RdBu)
            fig4.update_layout(height=400)
            st.plotly_chart(fig4, use_container_width=True)
        with col2:
            category_profit = df.groupby("product_category")["profit"].sum().reset_index()
            fig5 = px.bar(category_profit, x="product_category", y="profit",
                          title="Profitability by Category", color="profit", color_continuous_scale="Viridis")
    ...
    with tab4:
        ...
        with col1:
            ...
            fig6 = px.pie(payment_method, values="count", names="payment_method",
        ...
        with col2:
            ...
            payment_status["payment_received"] = payment_status["payment_received"].map({0: "Pending", 1: "Received"})
            fig7 = px.bar(payment_status, x="payment_received", y="count",
                          title="Payment Status Overview", color="payment_received",
    ...
    analysis_df = df[["customer_name", "sale_date", "product_category",
                       "buying_price", "selling_price", "profit", "profit_margin",
                       "amount_paid", "pending_amount"]].copy()
    analysis_df = analysis_df.rename(columns={
        "customer_name": "Customer", "sale_date": "Date", "product_category": "Category",
        "buying_price": "Buy Price", "selling_price": "Sell Price",
    ...
    with col_csv_a:
        st.download_button("📥 Download All Data as CSV", data=df.to_csv(index=False),
                           file_name=f"complete_sales_data_{date.today()}.csv", mime="text/csv",
                           use_container_width=True)
    with col_excel_a:
        excel_analytics = export_to_excel(df)
        st.download_button("📊 Download All Data as Excel", data=excel_analytics,
...
def customer_list():
    ...
    profit_by_customer = df.groupby('customer_name').apply(
        lambda x: (x['selling_price'] - x['buying_price']).sum()
    ).reset_index(name='total_profit')

    customer_summary = customer_summary.merge(profit_by_customer, on='customer_name', how='left')
    customer_summary.columns = ['Customer Name', 'Phone', 'Total Transactions',
    ...
    with col3:
        st.metric("Customers with Pending", len(customer_summary[customer_summary['Pending Amount (₹)'] > 0]))
    with col4:
    ...
    st.dataframe(customer_summary.style.format({
        'Total Sales (₹)': '₹{:,.2f}',
        'Pending Amount (₹)': '₹{:,.2f}',
        'Total Profit (₹)': '₹{:,.2f}'
    }), use_container_width=True, hide_index=True)
    ...
    col_csv, col_excel = st.columns(2)
    with col_csv:
        st.download_button("📥 Download Customer List (CSV)", data=customer_summary.to_csv(index=False),
                           file_name=f"customer_list_{date.today()}.csv", mime="text/csv",
                           use_container_width=True)
    with col_excel:
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        output.seek(0)
        st.download_button("📊 Download Customer List (Excel)", data=output,
    ...
    if selected:
        ...
        with col_d:
            st.metric("Profit from Customer", f"₹ {(customer_df['selling_price'] - customer_df['buying_price']).sum():,.0f}")

        history_df = customer_df[['sale_date', 'product_category', 'product_description',
                                   'selling_price', 'amount_paid', 'pending_amount', 'payment_method']].copy()
        history_df.columns = ['Date', 'Category', 'Description', 'Price (₹)', 'Paid (₹)', 'Pending (₹)', 'Payment Method']
...
def reminders_alerts():
    ...
    overdue = df[(df["pending_amount"] > 0) & ((datetime.now() - df["sale_date"]).dt.days > 30)]
    ...
    if len(high_value) > 0:
        st.dataframe(high_value[["customer_name", "sale_date", "product_category", "selling_price", "profit"]],
                     use_container_width=True, hide_index=True)
    else:
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if page == "🏠 Dashboard":
            ...
            docs = list(get_collection().find({}, {"_id": 0}))
            df_all = sales_to_df(docs)

            if not df_all.empty:
                col_info, col_export = st.columns([2, 1])
                with col_info:
                    st.info(f"💾 Ready to export {len(df_all)} sales records.")
                with col_export:
